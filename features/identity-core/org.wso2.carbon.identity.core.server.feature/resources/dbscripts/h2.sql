...
CREATE INDEX IDX_IDN_OAUTH_CONSUMER_SECRETS_CK ON IDN_OAUTH_CONSUMER_SECRETS (CONSUMER_KEY_ID);
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_CIBA_AUTH_CODE (
    ...
    RESOLVED_USER_ID CHAR(36),
    TENANT_ID INTEGER,
    AUTH_REQ_STATUS VARCHAR (100) DEFAULT 'REQUESTED',
    IDP_ID INTEGER,
    REQUESTED_ACTOR VARCHAR(255) DEFAULT NULL,
    UNIQUE(AUTH_REQ_ID),
...
CREATE TABLE IF NOT EXISTS IDN_PRESENTATION_DEFINITION (
    ...
    TENANT_ID     INTEGER       NOT NULL,
    PRIMARY KEY (ID),
    CONSTRAINT IDN_PD_TENANT_IDENTIFIER_UNQ UNIQUE (TENANT_ID, IDENTIFIER)
);

CREATE TABLE IF NOT EXISTS IDN_PD_CREDENTIAL (
    ID                      INTEGER       NOT NULL AUTO_INCREMENT,
    ...
    TENANT_ID               INTEGER       NOT NULL,
    PRIMARY KEY (ID),
    CONSTRAINT IDN_PD_CRED_DEF_IDENTIFIER_UNQ UNIQUE (DEFINITION_ID, IDENTIFIER),
    FOREIGN KEY (DEFINITION_ID) REFERENCES IDN_PRESENTATION_DEFINITION(ID) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS IDN_PD_CLAIM (
    ID              INTEGER       NOT NULL AUTO_INCREMENT,
    CREDENTIAL_ID   INTEGER       NOT NULL,
    CLAIM_PATH      VARCHAR(1024) NOT NULL,
    IS_MANDATORY    BOOLEAN       NOT NULL DEFAULT TRUE,
    TENANT_ID       INTEGER       NOT NULL,
    PRIMARY KEY (ID),
    FOREIGN KEY (CREDENTIAL_ID) REFERENCES IDN_PD_CREDENTIAL(ID) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS IDN_PD_ISSUER_CONFIG (
    ID                    INTEGER       NOT NULL AUTO_INCREMENT,
    CREDENTIAL_ID         INTEGER       NOT NULL,
    KEY_SOURCE_TYPE       VARCHAR(50)   NOT NULL,
...
CREATE INDEX IDX_IDN_AUTH_SSTR_TID_SID ON IDN_AUTH_SESSION_STORE (TENANT_ID, SESSION_ID);

-- IDN_AUTH_SESSION_META_DATA --
CREATE INDEX IDX_IDN_AUTH_SMD_PT_VAL_SID ON IDN_AUTH_SESSION_META_DATA (PROPERTY_TYPE, `VALUE`, SESSION_ID);

-- IDN_AUTH_TEMP_SESSION_STORE --
CREATE INDEX IDX_IDN_AUTH_TMP_SESSION_TIME ON IDN_AUTH_TEMP_SESSION_STORE (TIME_CREATED);

-- IDN_VP_SESSION_STORE --
CREATE TABLE IDN_VP_SESSION_STORE (
...
CREATE INDEX IDX_IDN_VP_SESSION_EXP ON IDN_VP_SESSION_STORE (EXPIRES_AT);
CREATE INDEX IDX_IDN_VP_SESSION_TID ON IDN_VP_SESSION_STORE (TENANT_ID);
...
CREATE INDEX IDX_REFRESH_TOKEN_BIND ON IDN_OAUTH2_REFRESH_TOKEN_BINDING (TOKEN_BINDING_REF);
CREATE INDEX IDX_REFRESH_TK_VALUE_TYPE ON IDN_OAUTH2_REFRESH_TOKEN_BINDING (TOKEN_BINDING_VALUE, TOKEN_BINDING_TYPE);
...
-- IDN_VC_NONCE --
CREATE INDEX IDX_IDN_VC_NONCE_EXPIRY_TIME ON IDN_VC_NONCE (EXPIRY_TIME);
...
CREATE INDEX IDX_IDV_TENANT_REGISTERED ON IDN_DEVICE (TENANT_ID, REGISTERED_AT);
CREATE INDEX IDX_IUD_USER_TENANT ON IDN_USER_DEVICE (USER_ID, TENANT_ID);
...
CREATE TABLE IDN_SQL_MONITOR_TEST (
    ID INTEGER NOT NULL,
    PRIMARY KEY (ID)
);
